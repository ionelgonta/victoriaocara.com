...
export async function GET(req: NextRequest) {
  try {
    ...
    let paintings;
    if (featured === 'true') {
      paintings = await db.getFeaturedPaintings();
    } else {
      paintings = await db.getAllPaintings();
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    const existingPainting = await db.getPaintingBySlug(slug);
    if (existingPainting) {
      return NextResponse.json({ error: 'A painting with this title already exists' }, { status: 400 });
    }

    const painting = await db.createPainting({ ...data, slug });
